...
class WebTest_Contribute_BraintreeContributionTest extends BaseBraintreeTestCase {
   ...
   /*     $this->open($this->sboxPath . "civicrm/admin/contribute/add?action=add&reset=1");
        ...
        $pageTitle = "Braintree Contribution Page";
        ...
        $this->clickAndWait('_qf_Settings_next-bottom');
        ...
        $checkBox = $table . "//tr//td/label[text()='Braintree']/../input[@type='checkbox']";
        ...
        $this->click('css=form#Premium div#id_Premiums.crm-block div.crm-accordion-wrapper');
        $this->type('premiums_nothankyou_label', 'No Thanks');	
        $this->waitForElementPresent('_qf_Custom_submit_savenext');
        $this->click('_qf_Custom_submit_savenext');
        $this->waitForPageToLoad('30000');
        ...
        $this->waitForElementPresent('_qf_Widget_upload_done-bottom');
        $this->click('_qf_Widget_upload_done-bottom');
        $this->waitForPageToLoad('30000');
*/
        $pageId = ContributionPage::create();
    ...
    function _doContributionPayment($pageId) {
        ...
        if ($this->isElementPresent('email-5')) {
            $this->type('email-5', 'vivek.arora@thetigroup.com');
        }
